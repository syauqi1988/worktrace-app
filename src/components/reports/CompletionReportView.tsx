...
function formatDateTime(d: string | null | undefined) {
  ...
  return `${dt.toLocaleDateString(loc, { day: 'numeric', month: 'short' })}, ${dt.toLocaleTimeString(loc, { hour: '2-digit', minute: '2-digit', hour12: false })}`;
}
...
export default function CompletionReportView({ report, job, customer, company }: CompletionReportViewProps) {
  ...
  function statusPill(status?: string | null) {
    ...
    if (s === 'accepted') return { label: t('completionView.statusAccepted'), dot: 'bg-emerald-600', bg: 'bg-emerald-50', text: 'text-emerald-700' };
    if (s === 'rejected') return { label: t('completionView.statusRejected'), dot: 'bg-rose-600', bg: 'bg-rose-50', text: 'text-rose-700' };
    if (s === 'submitted') return { label: t('completionView.statusSubmitted'), dot: 'bg-blue-600', bg: 'bg-blue-50', text: 'text-blue-700' };
    return { label: t('completionView.statusDraft'), dot: 'bg-slate-400', bg: 'bg-slate-100', text: 'text-slate-700' };
  }
  ...
  function PhotoCard({ src, kind, caption, fallbackTime }: { src: string; kind: string; caption?: string; fallbackTime?: string | null }) {
    return (
      ...
      <div className="bg-card border-b border-border px-4 sm:px-5 py-3.5 flex items-center justify-between gap-3">
        <div className="flex items-center gap-2.5 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <div className="text-[10px] text-muted-foreground font-mono">{t('completionView.title')}</div>
          </div>
        </div>
        ...
        <div className="flex items-start justify-between gap-3 mb-4">
          <div className="min-w-0">
            <h1 className="text-base sm:text-lg font-semibold text-foreground leading-tight tracking-tight">
              {job?.title || t('completionView.title')}
            </h1>
            <p className="text-[12px] text-muted-foreground mt-0.5">
              {job?.job_number && <span>{t('completionView.job')}: {job.job_number}</span>}
              {report.project_ref && <span> &nbsp;·&nbsp; {t('completionView.ref')}: {report.project_ref}</span>}
              <span> &nbsp;·&nbsp; {t('completionView.reportNo')}: {report.report_number}</span>
            </p>
          </div>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
          <MetaCell label={t('completionView.completionDate')} value={formatDate(report.completion_date)} />
          <MetaCell label={t('completionView.category')} value={job?.category || '—'} />
          <MetaCell label={t('completionView.location')} value={report.location_label || '—'} />
          <MetaCell label={t('completionView.preparedBy')} value={report.technician_name || '—'} />
        </div>
      </div>

      <div className="px-4 sm:px-5 py-5">
        <SectionHead label={t('completionView.customer')} />
        ...
        {report.work_description && (
          <>
            <SectionHead label={t('completionView.workDone')} />
        ...
        {report.materials_used && (
          <>
            <SectionHead label={t('completionView.materials')} />
            <div className="bg-muted/40 border border-border rounded-lg p-3 sm:p-4">
        ...
        {totalPhotos > 0 && (
          <>
            <SectionHead label={t('completionView.photos')} count={t('completionView.photosCount', { count: totalPhotos })} />
            <div className="grid grid-cols-2 gap-2.5 sm:gap-3">
              {before.map((src, i) => (
                <PhotoCard key={`b-${i}`} src={src} kind={t('completionView.before')} caption={beforeCaps[i]} fallbackTime={report.completion_date} />
              ))}
              {after.map((src, i) => (
                <PhotoCard key={`a-${i}`} src={src} kind={t('completionView.after')} caption={afterCaps[i]} fallbackTime={report.completion_date} />
              ))}
            </div>
          </>
        )}
        ...
        {checklist.length > 0 && (
          <>
            <SectionHead label={t('completionView.checklist')} count={t('completionView.checklistCount', { count: checklist.length })} />
              ...
              {checklist.map((item, i) => {
                ...
                return (
                  <div key={i} className="bg-card border border-border rounded-lg p-3 flex items-start gap-3">
                    ...
                    {!done && (
                      <span className="text-[10px] font-mono text-amber-700 bg-amber-50 px-2 py-0.5 rounded-full whitespace-nowrap self-start">{t('completionView.pending')}</span>
                    )}
                  </div>
                );
              })}
        ...
        <SectionHead label={t('completionView.signoff')} />
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
          <SignoffCard
            role={t('completionView.techRole')}
          ...
          />
          <SignoffCard
            role={t('completionView.custRole')}
            name={
              report.status === 'accepted'
                ? customer?.name || report.customer_signature || t('completionView.customerFallback')
                : report.customer_signature || t('completionView.waitingCust')
            }
            date={report.accepted_at}
            sub={
              report.status === 'accepted'
                ? t('completionView.viaWa')
                : report.status === 'rejected'
                ? t('completionView.rejectedSub')
                : t('completionView.notVerified')
            }
