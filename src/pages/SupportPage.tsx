...
export default function SupportPage() {
  ...
  return (
    <div className="p-4 md:p-6 space-y-6 max-w-3xl">
      ...
      {loading ? (
        <div className="text-center py-12 text-muted-foreground text-sm">Memuatkan...</div>
      ) : tickets.length === 0 ? (
        <div className="bg-card rounded-xl border border-border p-8 text-center space-y-3">
          ...
          <p className="text-base font-medium text-foreground">🎉 Tiada Isu Dilaporkan</p>
          <p className="text-sm text-muted-foreground">Anda belum ada tiket sokongan. Jika ada masalah, kami sedia membantu!</p>
            ...
            >
              ...
              <div className="flex items-center gap-2 text-xs text-muted-foreground">
                ...
                <span className={PRIORITY_STYLES[t.priority] || ''}>
                  {t.priority === 'low' ? 'Rendah' : t.priority === 'high' ? 'Tinggi' : t.priority === 'urgent' ? 'Urgent' : 'Normal'}
                </span>
                <span>•</span>
                <span>{new Date(t.created_at).toLocaleDateString('ms-MY', { day: 'numeric', month: 'short', year: 'numeric' })}</span>
              </div>
            </button>
          ))}
        </div>
